...
function runCase(name: string, runner: () => void): [string, unknown] {
  const context = setup();
  try {
    runner();
    renderPage();
    flushCoreContextEvents();
    return [name, [...context.hydrationData]];
  } finally {
    teardown(context);
  }
}
...
export function run(): Record<string, unknown> {
  ...
  results.set(
    ...runCase('multiple-root-instances', () => {
      function App() {
        return (
          <>
            <view />
